...
- **Fight battles you can win**. While based on anecdotal evidence, battling slightly lower lever pets seems easier, especially if you can keep going without healing. The less XP and void matter seems to be counterweighted by the fact that you can do more fights.

- **Don't use ineffective/low damage attacks**. Rather than wasting attack points on things like Gatekeep, keep up to 2 attack points which will roll over to the next turn. This will allow you to use more powerful attacks.

- **Use [type matching](/type-matching) to gain an advantage**. Each Voidpet has a different corresponding type, and each enemy has one too. You can use this to strategize and beat Voidpets if they have a weaker type. You can see all the type matches [here](/type-matching).
...
The Panic ball attack is an attack that does `mental damage` while Jitters does physical damage. For brevity, we'll ignore the amount of damage each attack does. If we go back to my skills, I do more damage with energy attacks rather than mental attacks, so I'll invoke the Panic ball attack.

### Use Stat Changing Attacks First

You should use attacks with effect before using entirely damage based attacks. If you plan to use multiple attacks, i.e. dark blast and panic ball, it would make sense to invoke Panic ball then Dark blast. Let's explore why:
